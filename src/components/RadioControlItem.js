...
export default function RadioControlItem({
  id,
  label,
  info,
  legend,
  ...
  const isChecked = selectedGraph === id
  // const canBeFiltered = !!controlState.range

  return (
    <>
      <ControlItemContainer htmlFor={id}>
        <Radio id={id} onChange={() => toggleLayer(id)} checked={isChecked} />
        {label}
